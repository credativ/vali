...
require "logstash/outputs/vali/entry"
require "logstash/outputs/vali/batch"
...
class LogStash::Outputs::Vali < LogStash::Outputs::Base
  include Vali
  config_name "vali"
  ...
  ## 'Maximum batch size to accrue before pushing to vali. Defaults to 102400 bytes'
  config :batch_size, :validate => :number, :default => 102400, :required => false

  ## 'Interval in seconds to wait before pushing a batch of records to vali. Defaults to 1 second'
  ...
  def register
    ...
    @logger.info("Vali output plugin", :class => self.class.name)
    ...
    loop do
      ...
      @mutex.synchronize do
        if !add_entry_to_batch(e)
          @logger.debug("Max batch_size is reached. Sending batch to vali")
          send(@batch)
          @batch = Batch.new(e)
        end
      end
  ...
  def max_batch_wait
    ...
    loop do
      ...
      if is_batch_expired
        @mutex.synchronize do
          @logger.debug("Max batch_wait time is reached. Sending batch to vali")
          send(@batch)
          @batch = nil
        end
      end
  ...
  def send(batch)
    ...
    res = vali_http_request(payload)
    if res.is_a?(Net::HTTPSuccess)
      @logger.debug("Successfully pushed data to vali")
    else
      @logger.debug("failed payload", :payload => payload)
    end
  end

  def vali_http_request(payload)
    req = Net::HTTP::Post.new(
      @uri.request_uri
    )
    req.add_field('Content-Type', 'application/json')
    req.add_field('X-Scope-OrgID', @tenant_id) if @tenant_id
    req['User-Agent']= 'vali-logstash'
    req.basic_auth(@username, @password) if @username
    req.body = payload

    opts = ssl_opts(@uri)

    @logger.debug("sending #{req.body.length} bytes to vali")
    retry_count = 0
    delay = @min_delay
    begin
